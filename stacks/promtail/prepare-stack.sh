...
_cfg="$(prepare_stack__expand_home_in_path "${HOME}/.config/promtail/promtail-config.yml")"
if [[ -f stack.env ]]; then
  _line=$(grep -E "^PROMTAIL_CONFIG_PATH=" stack.env 2>/dev/null | tail -1) || true
  if [[ -n "$_line" ]]; then
...
if [[ ! -f "$_cfg" ]]; then
  cp "$_PREPDIR/promtail-config.yml.example" "$_cfg"
  prepare_stack_msg "created ${_cfg} from promtail-config.yml.example — configure log paths and Loki URL."
else
