...
OSINT tool: collect a dossier on a person **by homelab-user only**, checking thousands of sites and gathering available info from profile pages. No API keys. Fork of Sherlock with profile parsing, recursive search, and report export (HTML, PDF, XMind).
...
3. In the web UI, enter one or more homelab-users, run the search, then view the graph/table and download reports (HTML, PDF, XMind).
...
The container runs the web interface (`maigret --web 5000`). For CLI-only runs (e.g. one-off homelab-user search), use the image directly:

```bash
docker run --rm -v maigret-reports:/app/reports soxoj/maigret:latest homelab-user --html
```
