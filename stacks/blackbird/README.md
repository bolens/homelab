...
OSINT tool to search for accounts by homelab-user or email across many sites (Sherlock-like, with extended coverage and report export). Supports PDF/CSV reports and optional AI-based profiling.
...
4. **Run homelab-user/email searches** (SANITIZED examples):
   ...
   ```bash
   # Simple homelab-user search (prints to console)
   docker compose run --rm blackbird --homelab-user johndoe
   ...
   docker compose run --rm blackbird --homelab-user johndoe --csv /results/johndoe.csv
   ```
