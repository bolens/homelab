...
_cfg="$(prepare_stack__expand_home_in_path "${HOME}/.config/loki/loki-config.yml")"
if [[ -f stack.env ]]; then
  _line=$(grep -E "^LOKI_CONFIG_PATH=" stack.env 2>/dev/null | tail -1) || true
  if [[ -n "$_line" ]]; then
...
if [[ ! -f "$_cfg" ]]; then
  cp "$_PREPDIR/loki-config.yml.example" "$_cfg"
  prepare_stack_msg "created ${_cfg} from loki-config.yml.example — review retention and storage settings."
else
