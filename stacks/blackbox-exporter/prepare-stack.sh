...
_cfg="$(prepare_stack__expand_home_in_path "${HOME}/.config/blackbox-exporter/blackbox.yml")"
if [[ -f stack.env ]]; then
  _line=$(grep -E "^BLACKBOX_CONFIG_PATH=" stack.env 2>/dev/null | tail -1) || true
  if [[ -n "$_line" ]]; then
...
if [[ ! -f "$_cfg" ]]; then
  cp "$_PREPDIR/blackbox.yml.example" "$_cfg"
  prepare_stack_msg "created ${_cfg} from blackbox.yml.example — edit probe targets before starting."
else
