...
1. Run `./prepare-stack.sh` from this directory (creates local config and
   ensures `ingress-public`, `media-automation`, and the private `media-services` network).
2. Edit `stack.env`:
...
| **Networks** | `ingress-public` for Caddy; `media-automation` for Lidarr; private `media-services` for Explo and trusted library clients |
