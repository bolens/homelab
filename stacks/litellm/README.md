...
1. `./prepare-stack.sh` — creates `stack.env`, **`~/.config/litellm/config.yaml`** (from `litellm_config.yaml.example` when missing), optional `caddy_snippet.conf`, ensures `monitor`.
...
| Client | Setting |
|--------|---------|
| **Open WebUI** | Through Caddy: API base **`https://<host>/api/v1`** (not `/v1` at host root). On Docker **`monitor`**: **`http://litellm:4000/v1`**. API key = `LITELLM_MASTER_KEY` or a LiteLLM virtual key. |
...
- Attach stack to external **`monitor`** network.
