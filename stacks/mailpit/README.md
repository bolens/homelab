...
Mailpit and Postfix share `mail-clients`. Mailpit also joins `ingress-admin` for its web UI. Apps send to `smtp-relay:587`; Postfix relays to Mailpit; Mailpit catches everything.
...
| **Network** | `mail-clients` for SMTP; `ingress-admin` for the web UI |
