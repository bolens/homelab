...
# Homelab override (cciucd image defaults: 5 attempts, 300s lockout).
# Edit here if you remount this file; image upgrades may require refreshing this patch.
...
def login():
    ...
    if login_failed_ip:
        ...
        if login_failed_ip >= _LOGIN_MAX_ATTEMPTS:
    ...
    if request.method == 'POST':
        ...
        if username is not None:
            ...
            if login_failed_user_id:
                ...
                if login_failed_user_id >= _LOGIN_MAX_ATTEMPTS:
            ...
            # login failed
                ...
                r_cache.expire('failed_login_ip:{}'.format(current_ip), _LOGIN_LOCKOUT_SEC)
                r_cache.incr('failed_login_user_id:{}'.format(username))
                r_cache.expire('failed_login_user_id:{}'.format(username), _LOGIN_LOCKOUT_SEC)
                #
