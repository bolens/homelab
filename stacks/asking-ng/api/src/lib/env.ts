...
export const appEnv = {
  ...
  wsMaxSubscribersPerPoll: Math.min(50_000, Math.max(10, Math.floor(env.WS_MAX_SUBSCRIBERS_PER_POLL))),
  wsFanoutMaxPerPollPerSec: Math.min(50_000, Math.max(1, Math.floor(env.WS_FANOUT_MAX_PER_POLL_PER_SEC))),
  ...
  llmExposePublicDetails: parseBoolish(env.LLM_EXPOSE_PUBLIC_DETAILS, env.NODE_ENV !== 'production'),
  llmGatewayToken: env.LLM_GATEWAY_TOKEN.trim(),
  llmRateLimitWindowMs: Math.min(Math.max(1_000, Math.floor(env.LLM_RATE_LIMIT_WINDOW_MS)), 3_600_000),
  llmRateLimitMaxPerWindow: Math.min(Math.max(1, Math.floor(env.LLM_RATE_LIMIT_MAX_PER_WINDOW)), 10_000),
  ...
  trustIpBurstVoteThreshold: Math.max(0, Math.min(500, Math.floor(env.TRUST_IP_BURST_VOTE_THRESHOLD))),
  trustChatBurstWindowSec: Math.min(300, Math.max(3, Math.floor(env.TRUST_CHAT_BURST_WINDOW_SEC))),
  trustChatBurstVoteThreshold: Math.max(0, Math.min(500, Math.floor(env.TRUST_CHAT_BURST_VOTE_THRESHOLD))),
  ...
  integrationWebhookTimeoutMs: Math.min(30_000, Math.max(500, Math.floor(env.INTEGRATION_WEBHOOK_TIMEOUT_MS))),
  ...
  authProxyGroupsHeader: env.AUTH_PROXY_GROUPS_HEADER.trim().toLowerCase() || 'x-auth-request-groups',
