...
export function normalizeMediaAttachment(raw: unknown): MediaAttachment | null {
  ...
  const preview_url = typeof r.preview_url === 'string' && r.preview_url.trim() ? r.preview_url.trim() : undefined;
...
export function normalizeMediaModeration(raw: unknown): MediaModeration {
  ...
  const reported_reason =
    typeof r.reported_reason === 'string' && r.reported_reason.trim() !== '' ? r.reported_reason.trim() : null;
  const last_action_at =
    typeof r.last_action_at === 'string' && r.last_action_at.trim() !== '' ? r.last_action_at.trim() : null;
  const last_actor = typeof r.last_actor === 'string' && r.last_actor.trim() !== '' ? r.last_actor.trim() : null;
