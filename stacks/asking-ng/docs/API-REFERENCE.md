...
| Session billing links | `GET /profile/billing` | JWT session; returns Polar portal/checkout URLs from `POLAR_*` env when configured |
...
| Distribution presets (creator UX) | `GET /poll/mine` data + public poll URLs | `My Polls` renders channel-ready share presets and CTA snippets by composing poll links with UTM params (`utm_source`, `utm_medium`, `utm_campaign`) |
