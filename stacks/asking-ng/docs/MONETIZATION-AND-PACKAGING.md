...
Enforcement surfaces: HTTP `429` / **`USAGE_LIMIT_API_RATE`**; WebSocket close or server message with stable codes; outbound poll webhooks shed with **`USAGE_LIMIT_POLL_WEBHOOK_DELIVERY`** (see [API-REFERENCE.md](API-REFERENCE.md)).
...
| API (partial, shipped) | `POST /poll` and **`POST /poll/:id/clone`**: when **`BILLING_ENFORCE_LIMITS=true`**, reject if non-archived owned polls ≥ cap for `users.billing_plan` (**`USAGE_LIMIT_ACTIVE_POLLS`**, HTTP **403**; details include `max`, `current`, `plan`). Clone now also enforces premium webhook automation and retention: if source poll carries non-empty `webhook_targets`, free workspaces get **`PLAN_LIMIT_AUTOMATION`**; if source poll carries custom `retention_ttl_days` or `retention_legal_hold=true`, free workspaces get **`PLAN_LIMIT_RETENTION`**; expired selfhost-pro licenses get **`BILLING_LICENSE_EXPIRED`** on those premium paths. **`PUT /poll/:id/vote`**: same flag rejects when new vote **rows** would push the poll **owner’s** non-quarantined vote count for the **UTC calendar month** over the plan cap (**`USAGE_LIMIT_VOTES`**, HTTP **403**; details include `max`, `current`, `plan`, `incoming`). **`GET /poll/:id/export`** and **`GET /profile/export`**: same flag rejects when completed export jobs for the **workspace** (poll **owner** `creator_user_id`, or signed-in user for profile export) in the **UTC calendar day** ≥ plan cap (**`USAGE_LIMIT_EXPORTS`**, HTTP **403**; details `max`, `current`, `plan`). Metering rows: `audit_logs.action=usage.data_export` with `details.workspace_user_id`. **`GET /profile/billing`** **`usage`** adds `exportsToday`, `maxExportsPerDay` plus prior fields, and now includes `usageLedger` (recent customer-visible metered rows, latest first; export jobs, shed telemetry rows `usage.campaign_attribution_shed` / `usage.poll_webhook_delivery_shed` / `usage.ws_fanout_shed`, and `usage.api_rate_limited` when tier HTTP burst limits trip), `usageLedgerDaily` (UTC-day grouped rollups for the same action taxonomy, trailing 14-day window by default), and `usageReconcile` (UTC-day parity: rollup vs uncapped raw `audit_logs` for exports, shed telemetry, and API rate-limit hits). **`billing.pastDue`** / **`billing.subscriptionStatus`** are included when Polar subscription is linked (`workspaces.polar_subscription_status`). Authenticated REST: **`USAGE_LIMIT_API_RATE`** (HTTP **429**) per-user ~2× sustained req/min tier burst (`api/src/lib/billingHttpRateLimit.ts`); metered rows `audit_logs.action=usage.api_rate_limited` with `details.workspace_user_id` (`api/src/lib/billingApiRateLimitLedger.ts`). Outbound **poll** integration webhooks: per-workspace UTC-minute delivery attempt caps (`api/src/lib/billingPollWebhookDelivery.ts`, log `poll.webhook.delivery_shed`, code **`USAGE_LIMIT_POLL_WEBHOOK_DELIVERY`**). **Campaign (UTM) attribution** on **`GET /poll/:id`**: per-workspace UTC-**day** cap on `impressionAttribution` bumps (`api/src/lib/billingCampaignAttributionQuota.ts`, log `poll.campaign_attribution_shed`, code **`USAGE_LIMIT_CAMPAIGN_ATTRIBUTION`**). Poll-live outbound WS fanout shedding now emits `audit_logs.action=usage.ws_fanout_shed` for customer-visible metering and reconcile parity (`api/src/lib/billingWsFanoutLedger.ts`). **Automation feature gates** when limits are enforced: **`PLAN_LIMIT_AUTOMATION`** (HTTP **403**) on **`PUT /poll/:id/moderation/batch`** and on non-empty `webhook_targets` for **`POST /poll`** / **`PUT /poll/:id`** (and clone-from-webhooked source polls) when workspace plan is `free` (details include `plan`, `required_plan`, `upgrade_hint`). **Custom retention policy** (`retention_ttl_days`) on **`POST /poll`** / **`PUT /poll/:id`**: blocked for `free` when limits are enforced (**`PLAN_LIMIT_RETENTION`**, HTTP **403**; details include `plan`, `required_plan`, `upgrade_hint`; `null` on update remains allowed to clear override; clone-from-custom-retention source polls now follows the same gate). Default **off**. Caps in `api/src/lib/billingLimits.ts`; checks in `api/src/lib/billingPollQuota.ts`, `api/src/lib/billingVoteQuota.ts`, `api/src/lib/billingExportQuota.ts`; vote count SQL in `api/src/controller/poll/voteOnPoll.repository.ts`. |
...
| Usage warnings (partial, shipped) | **`GET /profile/billing`** includes optional **`usage.warnings`** when any meter is ≥ **80%** or ≥ **95%** of its cap (`activePolls`, `votesThisMonth`, `dataExports`, **`pollWebhooksThisUtcMinute`**, **`campaignAttribution`** — values **`80`** / **`95`** for UI). **`usage.webhookDeliveriesThisUtcMinute`** / **`maxWebhookDeliveriesPerUtcMinute`** and **`campaignAttributionIncrementsToday`** / **`maxCampaignAttributionPerUtcDay`** expose live counters (in-process; resets on API restart). Does not block traffic. Builder in `api/src/lib/billingUsageWarnings.ts`. Client: severity banner on **Home**, **My Polls**, and **Developer** (`billing.usageNearLimit80` / `billing.usageNearLimit95`); Developer also lists per-meter hints. **Poll** page: localized hint when the live WebSocket closes with **`1008`** / **`USAGE_LIMIT_WS_SUBSCRIBERS`** (room cap). |
| UI | Mirrors limits; never sole enforcement. **Admin status now includes a downgrade simulator panel** that calls `POST /admin/billing/downgrade-simulate` to preview plan-change breakage (`USAGE_LIMIT_*` / `PLAN_LIMIT_*`) for a target owner before applying a plan change. |
| Regulated modules | No shadow endpoints; Tracks A/D/C only through dedicated gated APIs with jurisdiction / provider checks. |

**Retention operator knobs (shipped):** global sweep + default policy are driven by `POLL_RETENTION_TTL_DAYS_DEFAULT` (0 disables global auto-delete), `POLL_RETENTION_SWEEP_INTERVAL_SEC`, and `POLL_RETENTION_SWEEP_BATCH_SIZE` in `stack.env.example`; per-poll override remains `retention_ttl_days` (with `auto_delete_at_ms` exposed on poll reads).
...
| Webhooks | Polar uses [Standard Webhooks](https://www.standardwebhooks.com/); verify with `@polar-sh/sdk` `validateEvent` + `POLAR_WEBHOOK_SECRET`; idempotent handlers; DLQ + replay; alert on drift vs internal entitlements. **Scheduled reconcile shipped:** optional API-process cron (`POLAR_RECONCILE_INTERVAL_SEC`, `POLAR_RECONCILE_LIMIT`, requires `POLAR_ACCESS_TOKEN`) runs the same Polar API reconcile path as manual admin reconcile to repair drift automatically. |
| Failed payment | Grace ~7 days; define read-only vs hard-disable; email + in-app copy. |
| Self-host license | **Partial shipped:** env-backed `selfhost-pro` license state surfaced on `GET /profile/billing` (`billing.selfhostProLicense` with `active`/`grace`/`expired`/`unknown` from `SELFHOST_PRO_LICENSE_*` + grace hours). **Enforcement shipped (premium subset):** when state is `expired`, premium-gated endpoints now return **`BILLING_LICENSE_EXPIRED`** (replay/heatmap/moderation batch automation/webhook automation + retention fields). Remaining decisions/work: signed license issuance/verification source instead of env-only timestamps, rotation/revocation audit trail, full audit of premium/selfhost-only paths for expiry enforcement, and optional grace-expiring-soon renewal UX if product wants proactive prompts. |
...
| Customer visibility | **Partial shipped:** `GET /profile/billing` now includes `usage.usageLedger` (latest-first metered rows; export jobs, campaign-attribution/webhook-delivery/**ws-fanout** shed rows, and **`usage.api_rate_limited`** rows when tier HTTP burst limits apply), `usage.usageLedgerDaily` (UTC-day grouped rollups), and `usage.usageReconcile` (UTC-day parity checks vs uncapped raw `audit_logs` action counts for exports, shed telemetry, and API rate-limit hits). Current product focus: expose this payload in a customer-facing billing/settings view before expanding meter taxonomy further. Remaining decisions/work after that: downloadable/exportable customer ledger views, whether additional accepted billable actions (not just shed/export/rate signals) need user-visible ledger rows, and a rule that every newly-added ledger action gets matching reconcile coverage so billing history and raw logs stay explainable. |
...
Current implementation note: operators can place legal hold on poll retention (`retention_legal_hold`) and on non-poll sweeps (`audit_log_retention_legal_hold`, `moderation_rejected_vote_retention_legal_hold`) via `GET/POST /admin/retention-policy`, so auto-delete flows can be paused during investigations.
