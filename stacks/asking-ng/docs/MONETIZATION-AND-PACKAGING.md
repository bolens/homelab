...
| Usage warnings (partial, shipped) | **`GET /profile/billing`** includes optional **`usage.warnings`** when any meter is ≥ **80%** or ≥ **95%** of its cap (`activePolls`, `votesThisMonth`, `dataExports`, **`pollWebhooksThisUtcMinute`** for outbound poll webhook attempts in the current UTC minute — values **`80`** / **`95`** for UI). **`usage.webhookDeliveriesThisUtcMinute`** / **`maxWebhookDeliveriesPerUtcMinute`** expose the live counter (in-process; resets on API restart). Does not block traffic. Builder in `api/src/lib/billingUsageWarnings.ts`. Client: severity banner on **Home**, **My Polls**, and **Developer** (`billing.usageNearLimit80` / `billing.usageNearLimit95`); Developer also lists per-meter hints. **Poll** page: localized hint when the live WebSocket closes with **`1008`** / **`USAGE_LIMIT_WS_SUBSCRIBERS`** (room cap). |
