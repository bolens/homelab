...
- **My Polls list analytics:** Peak weekday (UTC) uses **`formatUtcWeekdayShort`**; peak clock hour uses **`formatUtcHourAtTopOfHour`** (same helpers as **Poll** metrics and **Admin** sparklines); top UTM campaign uses **`myPolls.topCampaign`**; poll **created** timestamp uses **`toLocaleString(localeTag)`**; when **`total_votes` > 0**, compact **UTC hour** and **weekday** bar sparklines (**`poll.metrics.hourly*`** / **`poll.metrics.weekday*`** + **`SparklineBars`**) mirror owner **Poll** metrics using **`GET /poll/mine`** **`hourly_votes_by_hour_utc`** / **`weekday_votes_by_dow_utc`** (non-quarantined votes).
- **Poll owner metrics (signed-in):** Velocity / completion / coverage / margin / time-to-first lines use **`poll.metrics.*`** keys; hourly range and sparkline bucket labels use **`formatUtcHourAtTopOfHour`** so **`poll.metrics.hourlyAxis`** and **`admin.dashboard.kpi.hourlySparklineAxis`** pass localized start/end without a second hard-coded `(UTC)` suffix; **owner-only** **`poll.metrics.optionHourlyTitle`** / **`poll.metrics.optionHourlyHint`** block shows **`option_hourly_votes_utc`** from **`GET /poll/:id`** (per configured answer, same 24 UTC clock-hour buckets as aggregate **`hourly_votes_by_hour_utc`**); **owner-only** **`poll.metrics.velocityByMinute*`** + horizontal scroll shows **`vote_velocity_by_minute_utc`** (export-shaped **`minute_utc`** / **`vote_count`**, up to **4000** most recent minutes; **`vote_velocity_by_minute_utc_truncated`** + **`poll.metrics.velocityByMinuteTruncated`** when the cap is hit; full series still via **`GET /poll/:id/export`**).
...
2. **Creator UX**: reusable distribution workflows — **in progress**: shared UTM helper (`withUtm`), presets/snippets/QR on **My Polls** and **Home** post-create (chrome i18n’d; **Home** postcard link, QR, snippet, and native share use preset-scoped UTMs aligned with **My Polls** chat / community / social intent), **Poll** owner “Sharing assets”; **native share** + **public poll URL** + **embed viewer URL** when a token exists; **OBS / browser-source** operator guide: [STREAMING-OBS-BROWSER-SOURCE.md](STREAMING-OBS-BROWSER-SOURCE.md) with in-app link (`docs.streamingObsBrowserSource`, optional build **`VITE_STREAMING_OBS_DOC_URL`**); **My Polls** list vote timing sparklines + **Poll** owner metrics strings + UTC helpers (**`formatUtcWeekdayShort`**, **`formatUtcHourAtTopOfHour`**, **`myPolls.topCampaign`**, **`poll.metrics.*`**); **Poll** owner **per-option UTC hour** (**`metrics.option_hourly_votes_utc`**) + **minute vote velocity** (**`metrics.vote_velocity_by_minute_utc`**, **`metrics.vote_velocity_by_minute_utc_truncated`**, **`poll.metrics.velocityByMinuteTruncated`**); **PollResults** overlay toolbar (**`poll.results*`**); next: denser chart presets / optional **per-option** minute splits; trust-layer **retention** or gamification per roadmap order.
