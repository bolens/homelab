...
1. Run `./prepare-stack.sh` (prepares config and ensures `ai-backend` plus `ingress-public`).
...
1. Deploy **litellm** on `ai-backend` and set `LITELLM_MASTER_KEY` in `stacks/litellm/stack.env`.
