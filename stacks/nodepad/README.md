...
1. Run the **Ollama** stack on **`monitor`** so `http://ollama:11434` resolves from the nodepad container (set `OLLAMA_ORIGIN` in `stack.env`; the example defaults to `http://ollama:11434`).
...
2. **Network:** Ensure the external Docker network **`monitor`** exists (`docker network create monitor` if needed), same as your other Caddy-backed stacks.
...
- **Internal:** `http://nodepad:3000` on the `monitor` network.
