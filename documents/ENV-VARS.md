...
- **shlink** – TZ (optional); DEFAULT_DOMAIN (required; short domain hostname, e.g. short.yourdomain.com); IS_HTTPS_ENABLED (true behind Caddy); GEOLITE_LICENSE_KEY (required for geolocation; free key at MaxMind); optional INITIAL_API_KEY, TRUSTED_PROXIES. No host ports; access via Caddy to shlink:8080. See stack `stack.env.example` and [Shlink env docs](https://shlink.io/documentation/environment-variables).
