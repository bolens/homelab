...
**Shared env file (TZ & locale):** To set `TZ` and locale in one place for many stacks (CLI and Portainer), see [SHARED-RESOURCES.md – Shared env file](SHARED-RESOURCES.md#1-shared-env-file-tz--locale).
...
- **loki** – TZ (optional). Copy `loki-config.yml.example` → `loki-config.yml`, `promtail-config.yml.example` → `promtail-config.yml`. No Caddy; add as Grafana data source (http://loki:3100).
...
- **outline** – TZ (optional); POSTGRES_*, URL (e.g. https://outline.yourdomain.com), SECRET_KEY, UTILS_SECRET; AWS_* for S3 (e.g. MinIO). No host ports; access via Caddy to outline:3000.
