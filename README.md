...
- **stacks/picard** — run `./prepare-stack.sh`; optionally set `PUID`/`PGID`, then confirm `PICARD_MUSIC_PATH` (default `/mnt/unraid/media/music`), `PICARD_SOULSEEK_INTAKE_PATH` (default `/mnt/unraid/media/downloads/soulseek`), and `PICARD_LIDARR_INTAKE_PATH` (default `/mnt/unraid/media/intake/lidarr`) in `stack.env`. Access via Caddy to `picard:5800`.
