...
Docker Compose stacks for self-hosting: reverse proxy, monitoring, auto-updates, and optional Cloudflare Tunnels.
...
| **stacks/caddy** | Caddy reverse proxy with automatic HTTPS (Let’s Encrypt) |
...
1. **Secrets and config** – Files like `.env`, `config.yml`, and `Caddyfile` are gitignored. Copy from the `.example` templates and fill in your values:
   ...
   - `stacks/caddy`: copy `Caddyfile.example` → `Caddyfile`
